...
export class VolunteerApiController {
  ...
  @Get('/tasks/accepted')
  @AccessControlList({ role: UserRole.VOLUNTEER, level: UserStatus.CONFIRMED })
  public async getAcceptedTasks(@Query() query: GetTasksSearchDto) {
    return this.tasksService.getTasksByStatus(TaskStatus.ACCEPTED, query);
  }

  @Get('/tasks/completed')
  @AccessControlList({ role: UserRole.VOLUNTEER, level: UserStatus.CONFIRMED })
  public async getCompletedTasks(@Query() query: GetTasksSearchDto) {
    return this.tasksService.getTasksByStatus(TaskStatus.COMPLETED, query);
  }
}
